...
class NNClass:
    ...
    nnrock = 'x'
    mname = ''
    def __init__(self,rock,mname):
        self.mname = mname
        self.nnrock = rock
        if Path(mname).is_file():
            self.model = pickle.load(open(mname,"rb"))
        else:
    ...
    def nnplay (self,board,rock,kholocs):
        ...
        if(self.mname == "perfect_model_80.pkl"):
            probarray = self.model.predict_proba(X)[:,1]
        else:
            probarray = self.model.predict_proba(X)[:,3:].sum(axis=1)
        #probarray = self.model.predict_proba(X)[:,6]
        #print(rx)
        #print(probarray)
        #print(f'choosing {rx[np.argmax(probarray)]}')
...
class Board:
    ...
    def play_n_validate(self,playfunc,rock):
        ...
        if(np.isin(u_rock,r_idx) and np.isin(u_place,b_idx)):
            ...
            if (self.showplay):
                bstr = ''.join(self.barr)
                printstr = f'Round:{len(self.rounds)}:{bstr} [{rock}:{u_rock}->{u_place}] '
...
if __name__ == "__main__":
    ...
    for games in range(no_games):
        ...
        nnmodel = NNClass(nn_rock,"perfect_model_90.pkl")
        #nnmodel2 = NNClass(rand_rock,"perfect_model_80.pkl")
        ...
        if(b.deadlock):
            deadlocks += 1
            print("Machines were in deadlock. Abandoning the game")
            continue
    ...
    print(f'NN won {ann_vict} out of {no_games} games with {suicide_games} with suicide and {deadlocks} deadlocks')
